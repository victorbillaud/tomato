...
import { getRandomElement } from "@/utils/utils-collection";
import { QR_CODE_SCAN_BASE_URL } from "@env";
...
export async function fetchQRCodes(supabase: SupabaseClient<Database>) {
	...
	const { data, error } = await supabase.from('qrcode').select(`*`).eq('user_id', user.id)
	if (error) throw error
	return data
}

export async function createQRCode(supabase: SupabaseClient<Database>, user_id: QRCodeInsert['user_id'], name?: QRCodeInsert['name']) {
	const { data, error } = await supabase
		.from('qrcode')
		.insert({
			user_id: user_id,
			name: name ?? `${getRandomElement(ADJECTIVES)} ${getRandomElement(FRUITS_VEGETABLES)}`,
		})
		.select()
		.single()
	if (error) throw error
