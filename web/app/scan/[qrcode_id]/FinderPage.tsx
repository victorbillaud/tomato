...
export default async function Scan({
  ...
  const ip_raw = headers().get('x-forwarded-for')?.split(':') ?? ['127.0.0.1'];
  const ip = ip_raw[ip_raw.length - 1];
  ...
  if (
    lastScanCookie &&
    new Date().getTime() - lastScanCookie.timestamp < 300000
  ) {
    ...
    const ipMetadata = await fetchLocationByIP(ip);
  ...
  if (!qrCode || error) throw new Error('QR Code not found');

  if (!qrCode?.item_id) throw new Error('QR Code not linked to an item');
  ...
  if (getPublicScanItemViewError) throw getPublicScanItemViewError;
        ...
        {qrCode?.item_id ? (
          ...
          <>
            <Text variant='body' className='text-center opacity-90'>
              How it seems you found a QR Code... But it&apos;s not linked to an
              item ! Try to find another one ! If you don&apos;t know{' '}
